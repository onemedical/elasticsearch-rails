...
To add the Elasticsearch integration for this model, require `elasticsearch_v2/model`
...
Instead of including the `ElasticsearchV2::Model` module directly in your model,
...
See the `ElasticsearchV2::Model` module documentation for technical information.
...
```ruby
ElasticsearchV2::Model.client = ElasticsearchV2::Client.new log: true
```
...
# => [#<ElasticsearchV2::Model::Response::Result:0x007 ... "_source"=>{"title"=>"Quick brown fox"}}>]
```
...
```ruby
ElasticsearchV2::Model.search('fox', [Article, Comment]).results.to_a.map(&:to_hash)
# => [
...
ElasticsearchV2::Model.search('fox', [Article, Comment]).records.to_a
...
query = ElasticsearchV2::DSL::Search.search do
  query do
    match :title do
      query 'fox dogs'
    end
...
The `ElasticsearchV2::Model` integration provides class methods to set up index settings and mappings.
...
the `ElasticsearchV2::Model::Callbacks` module in your model:
...
The automatic callbacks are implemented in database adapters coming with `ElasticsearchV2::Model`. You can easily
...
class Indexer
  ...
  Client = ElasticsearchV2::Client.new host: 'localhost:9200', logger: Logger
...
The `ElasticsearchV2::Model` module is fully compatible with any ActiveModel-compatible model, such as _Mongoid_:
...
the `ElasticsearchV2::Model` integration supports an "adapter" concept.
...
#
ElasticsearchV2::Model::Adapter.register(
  DataMapperAdapter,
  lambda { |klass| defined?(::DataMapper::Resource) and klass.ancestors.include?(::DataMapper::Resource) }
)
```

Require the adapter and include `ElasticsearchV2::Model` in the class:
...
More examples can be found in the `examples` folder. Please see the `ElasticsearchV2::Model::Adapter`
...
```ruby
ElasticsearchV2::Model.settings[:inheritance_enabled] = true
```
