...
ElasticsearchV2::Model.client.transport.logger.formatter = proc { |s, d, p, m| "\e[32m#{m}\n\e[0m" }
...
ElasticsearchV2::Model.client.indices.refresh index: ElasticsearchV2::Model::Registry.all.map(&:index_name)

puts "\n\e[1mArticles containing 'one':\e[0m", Article.search('one').records.to_a.map(&:inspect), ""

puts "\n\e[1mModels containing 'one':\e[0m", ElasticsearchV2::Model.search('one').records.to_a.map(&:inspect), ""
