...
class ElasticsearchV2::Model::MultipleTest < Test::Unit::TestCase
  ...
  context "Adapter for multiple models" do
    ...
    setup do
      @multimodel = ElasticsearchV2::Model::Multimodel.new(DummyOne, DummyTwo, Namespace::DummyTwo)
    end

    context "when returning records" do
      setup do
        @multimodel.class.send :include, ElasticsearchV2::Model::Adapter::Multiple::Records
        @multimodel.expects(:response).at_least_once.returns(stub(response: { 'hits' => { 'hits' => HITS } }))
      end

      should "keep the order from response" do
        assert_instance_of Module, ElasticsearchV2::Model::Adapter::Multiple::Records
