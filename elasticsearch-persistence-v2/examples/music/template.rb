...
initializer 'elasticsearch.rb', %q{
  ElasticsearchV2::Persistence.client = ElasticsearchV2::Client.new host: ENV['ELASTICSEARCH_V2_URL'] || 'localhost:9200'

  if Rails.env.development?
    logger           = ActiveSupport::Logger.new(STDERR)
    logger.level     = Logger::INFO
    logger.formatter = proc { |s, d, p, m| "\e[2m#{m}\n\e[0m" }
    ElasticsearchV2::Persistence.client.transport.logger = logger
  end
}.gsub(/^  /, '')
