RUBY_1_8 = defined?(RUBY_VERSION) && RUBY_VERSION < '1.9'

exit(0) if RUBY_1_8

$LOAD_PATH.unshift File.expand_path('../../../elasticsearch-model/lib', __FILE__) if File.exists? File.expand_path('../../../elasticsearch-model/lib', __FILE__)

require 'simplecov' and SimpleCov.start { add_filter "/test|test_/" } if ENV["COVERAGE"]

# Register `at_exit` handler for integration tests shutdown.
# MUST be called before requiring `test/unit`.
at_exit { ElasticsearchV2::Test::IntegrationTestCase.__run_at_exit_hooks } if ENV['SERVER']

if defined?(RUBY_VERSION) && RUBY_VERSION > '2.2'
  require 'test-unit'
  require 'mocha/test_unit'
else
  require 'minitest/autorun'
  require 'mocha/mini_test'
end

require 'shoulda-context'

require 'turn' unless ENV["TM_FILEPATH"] || ENV["NOTURN"] || defined?(RUBY_VERSION) && RUBY_VERSION > '2.2'

require 'ansi'
require 'oj'

require 'elasticsearch_v2/extensions/test/cluster'
require 'elasticsearch_v2/extensions/test/startup_shutdown'

require 'elasticsearch_v2/persistence'

module ElasticsearchV2
  module Test
    class IntegrationTestCase < ::Test::Unit::TestCase
      extend ElasticsearchV2::Extensions::Test::StartupShutdown

      startup  { ElasticsearchV2::Extensions::Test::Cluster.start(nodes: 1) if ENV['SERVER'] and not ElasticsearchV2::Extensions::Test::Cluster.running? }
      shutdown { ElasticsearchV2::Extensions::Test::Cluster.stop if ENV['SERVER'] && started? }
      context "IntegrationTest" do; should "noop on Ruby 1.8" do; end; end if RUBY_1_8

      def setup
        tracer = ::Logger.new(STDERR)
        tracer.formatter = lambda { |s, d, p, m| "#{m.gsub(/^.*$/) { |n| '   ' + n }.ansi(:faint)}\n" }
        ElasticsearchV2::Persistence.client = ElasticsearchV2::Client.new \
                                              host: "localhost:#{(ENV['TEST_CLUSTER_PORT'] || 9250)}",
                                              tracer: (ENV['QUIET'] ? nil : tracer)
      end

      def teardown
        ElasticsearchV2::Persistence.client.indices.delete index: '_all'
      end
    end
  end
end
