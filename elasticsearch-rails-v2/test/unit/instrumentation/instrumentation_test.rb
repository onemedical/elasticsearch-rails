...
class ElasticsearchV2::Rails::InstrumentationTest < Test::Unit::TestCase
  ...
  context "ActiveSupport::Instrumentation integration" do
    ...
    setup do
      @search = ElasticsearchV2::Model::Searching::SearchRequest.new ::DummyInstrumentationModel, '*'

      @client = stub('client', search: RESPONSE)
      DummyInstrumentationModel.stubs(:client).returns(@client)

      ElasticsearchV2::Rails::Instrumentation::Railtie.run_initializers
    end

    should "wrap SearchRequest#execute! with instrumentation" do
      s = ElasticsearchV2::Model::Searching::SearchRequest.new ::DummyInstrumentationModel, 'foo'
